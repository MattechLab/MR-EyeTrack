%% Batch-convert all reconstruction .mat volumes to NIfTI for subjects 1-15
...
for subject_num = 1:15
    ...
    if exist(refNifti, 'file') ~= 2 || exist(refJson, 'file') ~= 2
        fprintf('  skip — reference NIfTI/JSON not found\n');
        continue;
    end
    ...
    nJobs = nWoBin + numel(maskTypes) * 4 * 2;
    jobs  = cell(nJobs, 2);
    iJob  = 1;
    ...
    % Count pending jobs (MAT exists but NIfTI missing)
    nPending = 0;
    for k = 1:nJobs
        if exist(jobs{k,1}, 'file') == 2 && exist(jobs{k,2}, 'file') ~= 2
            nPending = nPending + 1;
        end
    end
...
switch className
    case 'uint8',   niftiDatatype = 2;
    case 'int16',   niftiDatatype = 4;
    case 'int32',   niftiDatatype = 8;
    case 'single',  niftiDatatype = 16;
    case 'double',  niftiDatatype = 64;
    case 'int8',    niftiDatatype = 256;
    case 'uint16',  niftiDatatype = 512;
    case 'uint32',  niftiDatatype = 768;
    case 'int64',   niftiDatatype = 1024;
    case 'uint64',  niftiDatatype = 1280;
    otherwise
