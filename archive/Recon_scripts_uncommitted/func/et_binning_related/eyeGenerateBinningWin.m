...
% How to select the number of bins?
  ...
  % Convert string answer to number
    ...
    %     for i = 1:nbins
    %         vi = logical(cMask(i, :));
    %         d = diff([false, vi, false]);
    %         idx_start = find(d == 1);
    %         idx_end = find(d == -1) - 1; % inclusive end indices

    %         if isempty(idx_start)
    %             % create a dummy (nan) patch to keep legend entry
    %             h(i) = patch([nan nan nan nan], [nan nan nan nan], colors(i, :), 'EdgeColor', 'none');
    %         else
    %             for s = 1:numel(idx_start)
    %                 is = idx_start(s);
    %                 ie = idx_end(s);
    %                 x1 = TimeStamp_s(is);
    %                 if ie < numel(TimeStamp_s)
    %                     x2 = TimeStamp_s(ie + 1);
    %                 else
    %                     x2 = TimeStamp_s(ie);
    %                 end
    %                 p = patch([x1 x2 x2 x1], [0 0 1 1], colors(i, :), 'EdgeColor', 'none');
    %                 if s == 1
    %                     h(i) = p; % use first segment as legend handle
    %                 end
    %             end
    %         end
    %     end
    %     legend(h, arrayfun(@(k) ['Bin ', num2str(k)], 1:nbins, 'UniformOutput', false), 'Location', 'eastoutside');
    %     ylim([-0.1 1.1]);
    %     xlabel('Time (seconds)');
    %     ylabel('Mask');
    %     title('Binning mask (single row; colored bars by bin)');
    %     hold off;
    % end
    ...
    if save_binning
        % save the figure
        figName = fullfile(ETDir, ['BinningMask_winLen', num2str(winLen), '_th', num2str(th_ratio), '_', mask_type, '.fig']);
        saveas(gcf, figName);
        disp(['Binning mask figure saved to: ', figName]);
        % save as png
        pngName = fullfile(ETDir, ['BinningMask_winLen', num2str(winLen), '_th', num2str(th_ratio), '_', mask_type, '.png']);
        saveas(gcf, pngName);
        disp(['Binning mask figure saved to: ', pngName]);
    end
