...
positions = [(0, 0.9), (0, -0.9), (0.9, 0), (-0.9, 0)]  # Define cross positions (up, down, right, left)
...
ioServer.getDevice('tracker').sendMessage("ET: prepare to start routine 'startET'")
# keep track of which components have finished
...
# set up handler to look after randomisation of conditions etc
T1w_LIBRE = data.TrialHandler(nReps=30.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
...
for thisT1w_LIBRE in T1w_LIBRE:
    ...
    ioServer.getDevice('tracker').sendMessage("ET: Prepare to start routine 'dots'")
    # keep track of which components have finished
    dotsComponents = [dot]
