% =====================================================
% Batch version of S2_eyeMask_t1_binning_pulseq_nomo.m
% Runs subjects 1-15, mask_type 'no-mo', no user prompts.
% Generates a single no-motion eMask per subject.
% =====================================================
clearvars; clc;
...
for subject_num = 1:15
    ...
    %% Auto-detect no-motion mask file
    maskFiles = dir(fullfile(ETDir, '*_nomo_mask.mat'));
    if isempty(maskFiles)
        warning('No nomo_mask found for %s, skipping.', subjectStr);
        continue;
    end
    ...
    fprintf('  [nomo] preserved: %d / %d readouts\n', sum(binMaskVec), NLin);
...
disp('Batch no-mo binning complete.');
