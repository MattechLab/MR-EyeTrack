...
% Runs subjects 1-15, mask_type 'filtered', no user prompts.
...
for subject_num = 1:15
    ...
    for idx_dir = 1:5
        ...
        maskFiles = dir(fullfile(ETDir, sprintf('*_ft_mask_%s.mat', dir_name)));

        if isempty(maskFiles)
            warning('No ft_mask_%s found for %s, leaving zeros.', dir_name, subjectStr);
            continue;
        end
...
disp('Batch filtered binning complete.');
