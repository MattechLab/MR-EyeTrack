% =====================================================
% Batch binning script (LEGACY 2018-2019 3T data) — fixation_ok mask
% Based on S2_eyeMask_t1_binning_pulseq_batch_fixation_ok.m
% Runs the 9 legacy subjects that have raw data, mask_type 'fixation-ok'.
% Generates one eMask per subject from the fixation_ok ET mask.
%
...
%% Config
mask_type  = 'fixation-ok';
maskGlob   = '*_fixation_ok_mask.mat';
...
for subject_num = 1:nSubjects
    ...
    %% Auto-detect fixation_ok mask file
    maskFiles = dir(fullfile(ETDir, maskGlob));
    if isempty(maskFiles)
        warning('fixation_ok mask not found for %s, skipping.', subjectStr);
        continue;
    end
    maskPath = fullfile(ETDir, maskFiles(1).name);
    fprintf('Mask [fixation_ok]: %s\n', maskFiles(1).name);
    ...
    fprintf('  [fixation_ok] preserved: %d / %d readouts (%.1f%%)\n', ...
...
disp('Batch fixation_ok binning (legacy data) complete.');
