% =====================================================
% Batch binning script (LEGACY 2018-2019 3T data) — tracking_loss mask
% Based on S2_eyeMask_t1_binning_pulseq_batch_tracking_loss.m
% Runs the 9 legacy subjects that have raw data, mask_type 'tracking-loss'.
% Generates one eMask per subject from the tracking_loss ET mask.
%
...
%% Config
mask_type  = 'tracking-loss';
maskGlob   = '*_tracking_loss_mask.mat';
...
for subject_num = 1:nSubjects
    ...
    %% Auto-detect tracking_loss mask file
    maskFiles = dir(fullfile(ETDir, maskGlob));
    if isempty(maskFiles)
        warning('tracking_loss mask not found for %s, skipping.', subjectStr);
        continue;
    end
    maskPath = fullfile(ETDir, maskFiles(1).name);
    fprintf('Mask [tracking_loss]: %s\n', maskFiles(1).name);
    ...
    fprintf('  [tracking_loss] preserved: %d / %d readouts (%.1f%%)\n', ...
...
disp('Batch tracking_loss binning (legacy data) complete.');
