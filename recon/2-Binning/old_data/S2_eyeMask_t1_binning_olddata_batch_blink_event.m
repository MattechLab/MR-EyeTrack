% =====================================================
% Batch binning script (LEGACY 2018-2019 3T data) — blink_event mask
% Based on S2_eyeMask_t1_binning_pulseq_batch_blink_event.m
% Runs the 9 legacy subjects that have raw data, mask_type 'blink-event'.
% Generates one eMask per subject from the blink_event ET mask.
%
...
%% Config
mask_type  = 'blink-event';
maskGlob   = '*_blink_event_mask.mat';
...
for subject_num = 1:nSubjects
    ...
    %% Auto-detect blink_event mask file
    maskFiles = dir(fullfile(ETDir, maskGlob));
    if isempty(maskFiles)
        warning('blink_event mask not found for %s, skipping.', subjectStr);
        continue;
    end
    maskPath = fullfile(ETDir, maskFiles(1).name);
    fprintf('Mask [blink_event]: %s\n', maskFiles(1).name);
    ...
    fprintf('  [blink_event] preserved: %d / %d readouts (%.1f%%)\n', ...
...
disp('Batch blink_event binning (legacy data) complete.');
