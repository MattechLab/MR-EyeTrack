function [twix_obj, rawData] = fReadSiemensRawData_FreeRunning( filepathRawData )
%--------------------------------------------------------------------------
%
...
%--------------------------------------------------------------------------
  ...
  % Add ReadRawDataSiemens directory to matlab path
    ...
    addpath([tmpdir filesep 'mapVBVD' filesep 'mapVBVD_multiRAID']);
...
%         
%         fprintf('... read noise data ...\n');
%         noise = twix_obj_multi{1}.noise{''};
% 
%         fprintf('... read ref data ...\n');
%         refscan = twix_obj_multi{1}.image{''};
%         refscan = permute(refscan,[1 3 4 2 5]);
%         FT = cFT([1,2]);
%         im_ref  = FT' * refscan;
% 
%         im5D(abs(im_ref));
