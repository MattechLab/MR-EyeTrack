function twix_obj = mapVBVD_jy(filename,varargin)

%  Reads Siemens raw .dat file from VB/VD MRI raw data.
%
%  Requires twix_map_obj_jy.m
%
...
%                          twix_map_obj_jy.m now stores the memory position of
...
%%%%% Parse varargin %%%%%
    ...
    twix_obj.image      = twix_map_obj_jy(arg,'image',filename,version);
    twix_obj.noise      = twix_map_obj_jy(arg,'noise',filename,version);
    twix_obj.phasecor   = twix_map_obj_jy(arg,'phasecor',filename,version);
    twix_obj.refscan    = twix_map_obj_jy(arg,'refscan',filename,version);
    twix_obj.refscanPC  = twix_map_obj_jy(arg,'refscan_phasecor',filename,version);
    twix_obj.RTfeedback = twix_map_obj_jy(arg,'rtfeedback',filename,version);    
    twix_obj.phasestab  = twix_map_obj_jy(arg,'phasestab',filename,version);
...
function [mdh mask] = evalMDHvb(fid,cPos)
    ...
    %----------------------------------------------------------------------
    % JY START (2013-11-06) Export ECG PMU time stamp and Time Stamp
    %----------------------------------------------------------------------
    mdh.lMeasUID                   = fread (fid,1,'int32');     % measurement user ID
    mdh.ulScanCounter              = fread (fid,1,'uint32');    % scan counter [1...]
    mdh.ulTimeStamp                = fread (fid,1,'uint32');    % Time stamp [2.5 ms ticks since 00:00]
    mdh.ulPMUTimeStamp             = fread (fid,1,'uint32');    % PMU time stamp [2.5 ms ticks since last trigger]
    %mdh.lMeasUID                   = fread (fid,1,'long');     % measurement user ID
    %mdh.ulScanCounter              = fread (fid,1,'ulong');    % scan counter [1...]
    %mdh.ulTimeStamp                = fread (fid,1,'ulong');    % Time stamp [2.5 ms ticks since 00:00]
    %mdh.ulPMUTimeStamp             = fread (fid,1,'ulong');    % PMU time stamp [2.5 ms ticks since last trigger]
    % ------------------------------------------------------------------------------------------
    % JY END (2013-11-06) Export PMU time stamp and Time Stamp
    %----------------------------------------------------------------------    
...
%     if mdh.ulPackBit
    % it seems that the packbit is not always set correctly
    if ~mask.MDH_SYNCDATA && mdh.ulDMALength~=0
