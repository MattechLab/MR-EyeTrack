classdef twix_map_obj_simo < handle
...
%
    ...
    properties(GetAccess='public', SetAccess='public')
        ...
        % [Simo 2013 02 01 - ECG Time Stamp ] ------------------
        TimeStamp
        PMUTimeStamp
        % ------------------------------------------------------ 
        
        % [Simo 2014 11 17 - UTE Readout OffCenter ] -----------
        fReadOutOffcenter
        % ------------------------------------------------------ 
    ...
    methods
        % Constructor:
        function this = twix_map_obj_simo(arg,dataType,fname,version)
        ...
        function this = readMDH(this,mdh,filePos)
            ...
            % ---------------------- [Simo 2013 01 31] ----------------------------------
            % Line number = Current acquisition counter, in order to compensate for the overflow of the line counter in the mdh
            cLin = cAcq;
            % ------------------------------------------------------------------------------
            ...
            if cAcq > this.currentAlloc
                ...
                % [Simo 2013 02 01 - ECG Time Stamp ] ------------------
                this.TimeStamp    = cat(2, this.TimeStamp   , zeros(1,this.allocSize,'double'));
                this.PMUTimeStamp = cat(2, this.PMUTimeStamp, zeros(1,this.allocSize,'double'));
                % ------------------------------------------------------
                
                % [Simo 2014 11 17 - UTE Readout OffCenter ] -----------
                this.fReadOutOffcenter = cat(2, this.fReadOutOffcenter  , zeros(1,this.allocSize,'single'));
                % ------------------------------------------------------
            ...
            % [Simo from Jean2014 12 05] Overflow managment, Double echo compatible
            this.Lin        (cAcq) = cLin;
%             if (cAcq<=1 || this.Lin (cAcq-1) <= cLin)
%                 this.Lin        (cAcq) = cLin;
%             else
%                 this.Lin        (cAcq) = 65536 + cLin;
%             end
            ...
            % [Simo 2013 02 01 - ECG Time Stamp ] ------------------
            this.TimeStamp  (cAcq) = mdh.ulTimeStamp;       % time since midnight
            this.PMUTimeStamp(cAcq)= mdh.ulPMUTimeStamp;    % time since the last Rwave
            % ------------------------------------------------------  
            
            % [Simo 2014 11 17 - UTE Readout OffCenter ] -----------
            this.fReadOutOffcenter (cAcq) = mdh.fReadOutOffcenter;
            % ------------------------------------------------------ 
        ...
        function this = clean(this)
            ...
            this.PMUTimeStamp= this.PMUTimeStamp(1:this.NAcq);
            % ------------------------------------------------------
            % [Simo 2014 11 17 - UTE Readout OffCenter ] -----------
            this.fReadOutOffcenter = this.fReadOutOffcenter (1:this.NAcq);
            % ------------------------------------------------------
            ...
            % [Simo 2014 12 05  Multi Echo UTE+GRE ] ----------------------------------
            % Now there are different values of NCol, depending on the echo selected
            if this.NEco == 1 % (Original Code)
                this.NCol = this.NCol(1); % assuming that all NCol values are the same when there is only one echo
            end
            % --------------------------------------------------------------------------
            ...
            % [Simo 2014 12 05  Multi Echo UTE+GRE ] ----------------------------------
            if this.NEco == 1
                this.fullSize = [ this.NCol this.NCha NLinAlloc NParAlloc... Original Line
                    this.NSli this.NAve this.NPhs this.NEco...
                    this.NRep this.NSet this.NSeg this.NIda...
                    this.NIdb this.NIdc this.NIdd this.NIde ];
            else
                this.fullSize = [ max(this.NCol(:)) this.NCha NLinAlloc NParAlloc...
                    this.NSli this.NAve this.NPhs this.NEco...
                    this.NRep this.NSet this.NSeg this.NIda...
                    this.NIdb this.NIdc this.NIdd this.NIde ];
            end
           % --------------------------------------------------------------------------
            ...
            if this.arg.removeOS
                ...
                this.dataSize(1) = max(this.NCol(:))/2;
            end
            ...
            nByte = this.NCha*(this.freadInfo.szChannelHeader +...
                    8*max(this.NCol(:)));
            ...
            if this.NEco == 1
                ...
                % reshape size
                this.freadInfo.shape = [max(this.NCol(:))+this.freadInfo.szChannelHeader/8 ...
                    , this.NCha];
                % we need to cut MDHs from fread data
                this.freadInfo.cut   = this.freadInfo.szChannelHeader/8+1 ...
                    : max(this.NCol(:))+this.freadInfo.szChannelHeader/8;
            end       
            % --------------------------------------------------------------------------              
        ...
        function varargout = subsref(this, S)
            ...
            if (bRemoveOS)
                ...
                cutOS        = max(this.NCol(:))/4+1:max(this.NCol(:))*3/4;
                % --------------------------------------------------------------------------
            ...
            for k=1:numel(mem)
                ...
                if this.NEco == 1
                    ...
                    % %  readShape can be different for each echo
                    newreadShape = [this.NCol(cIxToRaw(k))+this.freadInfo.szChannelHeader/8 ...
                        , this.NCha];
                    % we need to cut MDHs from fread data
                    newreadCut   = this.freadInfo.szChannelHeader/8+1 ...
                        : this.NCol(cIxToRaw(k))+this.freadInfo.szChannelHeader/8;
                    
                    % raw = reshape( complex(raw(:,1), raw(:,2)), readShape);
                    % raw = raw(readCut,:);
                    raw = raw(1:1:(newreadShape(1)*newreadShape(2)),:);
                    raw = reshape( complex(raw(:,1), raw(:,2)), newreadShape);
                    raw = raw(newreadCut,:);
                    % pad with zeros to ensure data consistency
                    raw = [raw; zeros(length(readCut) - length(newreadCut), size(raw,2))];
                end
                % --------------------------------------------------------------------------
