classdef twix_map_obj_JH_SOS < handle
...
methods
    % Constructor:
    function this = twix_map_obj_JH_SOS(arg,dataType,fname,version,rstraj)
...
%             end
            if this.NAcq > 2^16             % JY
                ...
                this.Lin(2^17+1:end) = this.Lin(2^17+1:end) + 2^16; % SR & LDS
                this.Lin(2^17+2^16+1:end) = this.Lin(2^17+2^16+1:end) + 2^16; % SR & LDS
            end
...
%         end
    ...
    function this = clean(this)
        ...
        % SvR: FOR STACK OF STARS
%         this.NLin = unique(this.NLin);
        ...
        NLinAlloc = max(1, (this.NLin/this.NPar) - this.skipLin);
