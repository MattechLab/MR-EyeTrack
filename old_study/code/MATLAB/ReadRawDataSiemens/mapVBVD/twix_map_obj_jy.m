classdef twix_map_obj_jy < handle
...
%--------------------------------------------------------------------------
% JY START (2013-11-11) Change parameter access to public (required to
% modify the data dimension after permutation - to follow Jerome's 
% convention)
%--------------------------------------------------------------------------
    properties(GetAccess='public', SetAccess='public')
    %properties(GetAccess='public', SetAccess='protected')
% -------------------------------------------------------------------------
% JY END (2013-11-11) Change parameter access to public
%--------------------------------------------------------------------------
        % properties:
        filename
        softwareVersion
        ...
        %----------------------------------------------------------------------
        % JY START (2013-11-06) ECG PMU time stamp and Time Stamp
        %----------------------------------------------------------------------
        TimeStamp       % Time since 00.00
        PMUTimeStamp    % time since last PMU trigger
        % ------------------------------------------------------------------------------------------
    ...
    methods
        % Constructor:
        function this = twix_map_obj_jy(arg,dataType,fname,version)
        ...
        function this = readMDH(this,mdh,filePos)
            ...
            if cAcq > this.currentAlloc
                ...
                %this.PMUTimeStamp = cat(2, this.PMUTimeStamp, alloc);
                %----------------------------------------------------------
                % JY END (2013-11-06) Export PMU time stamp and Time Stamp
                %----------------------------------------------------------
        ...
        function this = clean(this)
            ...
            % JY START (2013-11-06) ECG PMU time stamp and Time Stamp
            %--------------------------------------------------------------
            this.TimeStamp   = this.TimeStamp  (1:this.NAcq);    % time since midnight
            this.PMUTimeStamp= this.PMUTimeStamp(1:this.NAcq);  % time since the last Rwave
            %--------------------------------------------------------------
            % JY END (2013-11-06) Export PMU time stamp and Time Stamp
            %--------------------------------------------------------------
