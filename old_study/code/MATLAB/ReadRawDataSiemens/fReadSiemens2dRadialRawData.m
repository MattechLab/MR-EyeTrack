function [twix_obj, rawData] = fReadSiemens2dRadialRawData( filepathRawData )
%--------------------------------------------------------------------------
%
%   fReadRawDataSiemens     read raw data from Siemens scanner
%
%     [twix_obj rawData] = fReadSiemens2dRadialRawData( filepathRawData );
%
...
%--------------------------------------------------------------------------
  ...
  % Add ReadRawDataSiemens directory to matlab path
    ...
    fprintf('Start reading Siemens 2D radial raw data on %s\n', datestr(now));
    tic;
  ...
  %     accounts for the oversampling in the readout direction.
  %
  % Permute data to follow convention [Np Nseg Nshot Nc], i.e.,
  %  1) Columns
  %  2) Lines 
  %  3) Slices
  %  4) Channels/Coils
  ...
  % Permute dimension to follow Jerome's convention
    ...
    order_jy = [1 3 5 2 7 4 6 8 9 10 11 12 13 14 15 16];
    ...
    twix_obj.image.sqzDims  = tmpCell;
...
%--------------------------------------------------------------------------
  ...
  % Permuting raw data to appropriate format
  %     Raw data format = 2*Np x Ns x Nc
  %     where Np is the number of readout point, Nc is the number of
  %     channels, Ns is the total number of shots, and the factor 2 
  %     accounts for the oversampling in the readout direction.
  ...
  % Permute dimension to follow Jerome's convention
    fprintf('... permute raw data ...\n');
    rawData = permute( rawData, [1 3 5 7 2 4 6 8 9 10 11 12 13 14 15 16] );
