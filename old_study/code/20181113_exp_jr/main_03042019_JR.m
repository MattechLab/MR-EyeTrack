...
% fid = fopen('LA_samples_20190325.txt','rt');
...
assert(isempty(find(X_coord == 0, 1)));
...
mean_Mean_coordinates = mean(Mean_coordinates,1);
shiftVec = mean_Mean_coordinates - stimuli_centers(1,:);
shiftMat = repmat( shiftVec, [size(Mean_coordinates,1),1] );
Mean_coordinates = Mean_coordinates - shiftMat;

% indexes_for_reordering = zeros(number_of_stimuli,1);
[indexes_clustering, C] = kmeans(Mean_coordinates, number_of_stimuli,'Replicates',iter,'Start',repmat(stimuli_centers,[1,1,iter]));
indexes_clustering_ordered = zeros(size(indexes_clustering));
l = length(indexes_clustering);

% meanC = mean(C,1);
% shiftVec = meanC - stimuli_centers(1,:);
% shiftMat = repmat( shiftVec, [number_of_stimuli,1] );
% C = C - shiftMat;

for j = 1:number_of_stimuli  
     supp = C-repmat(stimuli_centers(j,:),[number_of_stimuli,1]); % supp = C-stimuli_centers(j,:);
     [minValue,closestIndex] = min(sqrt(supp(:,1).^2 + supp(:,2).^2)); % [minValue,closestIndex] = min(sqrt(sum(supp(:,1).^2 + supp(:,2).^2,2)));
%      indexes_for_reordering(j) = indexes_clustering(closestIndex);
     for k = 1:l
         if indexes_clustering(k) == closestIndex
             indexes_clustering_ordered(k) = j;
         end
     end
end

indexes_clustering = indexes_clustering_ordered;
...
%msgbox('Taaanti auguri di buon compleanno B! =)')
